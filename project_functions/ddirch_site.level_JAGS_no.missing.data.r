...
#' @param silent.jags if jags should be printing status to screen. Useful for running in parallel sometimes.
#'
...
#'
#' @examples
site.level_dirlichet_jags     <- function(y, x_mu, 
                                          adapt = 500, burnin = 1000, sample = 2000, 
                                          n.chains = 3, parallel = F, silent.jags = F){
  #Load some important dependencies.
  source('project_functions/crib_fun.r')
  source('project_functions/sd_to_precision.r')
  source('project_functions/precision_matrix_match.r')
